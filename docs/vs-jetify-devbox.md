# dev-box vs. Jetify Devbox

["Devbox" by Jetify](https://github.com/jetify-com/devbox) is a mature, popular (~12k stars)
open-source tool that solves a problem adjacent to `dev-box`'s: reproducible developer
environments. The names are close enough that it's worth being precise about how the two
actually differ, so this document exists to keep that comparison honest and in one place
rather than scattered across READMEs and issues.
...
| **Language / maturity** | Go, ~12k stars, backed by a company, production-used | Rust, brand new, pre-release, not yet validated against a real Distrobox setup end-to-end |
...
`dev-box` solves a different problem: "give me a full, real, isolated Linux *container* I can
...
the other — e.g. you need an entirely different distro's toolchain intact (pick `dev-box`), or
...
1. **Pin the resolved image, not just a floating tag.**
   ...
   later. `dev-box`'s `image = "ubuntu:24.04"` is a moving target. A `dev-box.lock`
   (or a `[dev-environment] image_digest=` field, written automatically after a successful
   `dev-box up`) would let `dev-box up` reproduce the exact same image on a teammate's
   machine, while still letting `dev-box up --pull`/similar intentionally refresh it.

2. **Auto-detect and offer to install missing dependencies.**
   Devbox bootstraps Nix automatically if it's missing. `dev-box` currently just prints an
   install link and exits. A `dev-box doctor` (or automatic prompt on first `up`) that
...
3. **A literal, declarative `env` section, distinct from `forward_env`.**
   `dev-box`'s `forward_env` intentionally only ever carries variable *names* (the values
   ...
   (e.g. `EDITOR=vim`, `NODE_ENV=development`). `dev-box` has no equivalent today short of an
...
4. **An explicit `include=` composition option, alongside the automatic file-cascade.**
   `dev-box`'s N-layer cascade is automatic and file-location-based (global → project →
   ...
   across unrelated projects easier than copy-pasting `devbox.ini` fragments.
...
5. **An export path back to plain Dev Containers / Dockerfile, for teams that need it.**
   ...
   source, so teams aren't fully locked into Devbox's own runtime. A `dev-box export
   --devcontainer` / `dev-box export --dockerfile`, translating the merged INI into those
   formats, would give `dev-box` users the same escape hatch — useful for onboarding a
...
6. **Community/contribution scaffolding.**
   ...
   crediting Nix. `dev-box` should do the same as it grows past a single-maintainer
